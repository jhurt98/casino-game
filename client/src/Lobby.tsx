...
function Lobby() {
    ...
    const playerList = players.map((player) => {
        ...
        return (
            <div style={{ alignSelf: "start" }} key={"list" + player.id}>
                <span id={dotId}>&#9679;</span>
                {player.name}
            </div>
        );
    ...
    return (
        <div className="lobby">
            {showToast && <div className={`copy-toast ${showToast ? "show" : ""}`}>Room ID copied to clipboard! ✓</div>}
            <div className="lobbyForm">
                ...
                {roomId === "" && (
                    <button className="lobbyForm-btn" type="submit" onClick={handleJoinRoom}>
                        Join Room
                    </button>
                )}
                <button className="lobbyForm-btn" type="button" onClick={roomId ? leaveRoom : handleCreateRoom}>
                    {roomId ? "Leave Room" : "Create Room"}
                </button>
                {roomId !== "" && (
                    <button className="lobbyForm-btn" type="button" onClick={startGame}>
                        Start Game
                    </button>
                )}
